...
class RapidiarioContractWindow(ModernWindow):
    def __init__(self, parent):
        super().__init__(parent, title="Contrato de Préstamo Rapidiario", width=900, height=700)
        self.loan_data = None
        self.create_widgets()

    def create_widgets(self):
        self.create_header("⚡ Contrato de Préstamo Rapidiario")
        ...
        tk.Label(loan_card, text="2. Seleccionar Préstamo (Rapidiario)", font=("Segoe UI", 12, "bold"), bg=self.card_bg, fg=self.text_color).pack(anchor='w', padx=15, pady=(15, 10))
    ...
    def search_client(self, event=None):
        ...
        # Filter clients who have at least one 'rapidiario' loan
        sql = """
    ...
    def load_loans(self, client_id):
        ...
        # Only Rapidiario loans
        sql = """
            SELECT id, amount, start_date, status 
            FROM loans 
            WHERE client_id = ? AND loan_type = 'rapidiario'
    ...
    def load_contract_data(self, loan_id):
        ...
        preview += f"PRÉSTAMO RAPIDIARIO:\nMonto: S/ {self.loan_data['amount']:.2f}\n"
        preview += f"Interés: {self.loan_data['interest_rate']}%\n"
        preview += f"Fecha: {self.loan_data['start_date']}\n"
        
        # Calculate total to pay (Amount + 20% usually for Rapidiario, but let's use interest rate)
        # Rapidiario logic: Amount * 1.20 usually (20% interest for 24 days)
        # But we should use the stored interest rate.
        
        total_pagar = self.loan_data['amount'] * (1 + self.loan_data['interest_rate']/100)
        preview += f"Total a Pagar: S/ {total_pagar:.2f}\n"
    ...
    def generate_contract(self):
        ...
        try:
            ...
            # Total
            total_pagar = monto * (1 + tasa_total/100)
            data['total_pagar'] = f"{total_pagar:,.2f}"
            data['total_texto'] = numero_a_letras(total_pagar)
            
            # Calculate workdays excluding Sundays (same logic as calculator)
            # Import the calculator function
            from utils.loan_calculator import calcular_dias_laborables
            from datetime import datetime as dt
            
            # Get loan start date
            if isinstance(self.loan_data['start_date'], str):
                fecha_inicio = dt.strptime(self.loan_data['start_date'], '%Y-%m-%d').date()
            else:
                fecha_inicio = self.loan_data['start_date']
            
            # Calculate workdays (excludes Sundays)
            dias_plazo = calcular_dias_laborables(fecha_inicio, 30)
            cuota_diaria = total_pagar / dias_plazo
            data['cuota_diaria'] = f"{cuota_diaria:,.2f}"
            data['cuota_texto'] = numero_a_letras(cuota_diaria)
            data['dias_plazo'] = dias_plazo
            
            # Generate PDF
            generator = PDFGenerator()
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"contrato_rapidiario_{self.loan_data['id']}_{timestamp}.pdf"
            filepath = os.path.join(generator.reports_dir, filename)
            
            path = generator.generate_rapidiario_contract(filepath, data)
